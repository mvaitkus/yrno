...
export const loadForecast = async (maybeSpot: string): Promise<Forecast> => {
	...
	if (currentRes.ok) {
		...
		if (isCoastal) {
			...
			waves = matchingInterval?.sea?.wave?.height;
			waveDirection = matchingInterval?.sea?.wave?.direction;
		}
		...
		days.set(currentDt.getDate(), {
			dateStr: currentDt.toLocaleDateString('lt-LT', {
				weekday: 'long',
				day: 'numeric',
				month: 'long'
			}),
	...
	return {
		created: new Date(forecastJson.created).toLocaleTimeString('lt-LT', {
			hour: '2-digit',
			minute: '2-digit'
		}),
